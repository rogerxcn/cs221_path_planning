...
def desc_agent(env, label_data, mode=None):
    ...
    def max_action(posX, posY):
        max_v = -1
        max_a = None

        if valid_next(posX-1, posY):
            if label_data[posX-1][posY] > max_v:
                max_v = label_data[posX-1][posY]
                max_a = env.UP
        if valid_next(posX+1, posY):
            if label_data[posX+1][posY] > max_v:
                max_v = label_data[posX+1][posY]
                max_a = env.DOWN
        if valid_next(posX, posY-1):
            if label_data[posX][posY-1] > max_v:
                max_v = label_data[posX][posY-1]
                max_a = env.LEFT
        if valid_next(posX, posY+1):
            if label_data[posX][posY+1] > max_v:
                max_v = label_data[posX][posY+1]
    ...
    while env.num_unvisited_nodes() > 0:
        ...
        max_a, max_v = max_action(aloc[0], aloc[1])


        if mode == "astar" and max_a is None:
